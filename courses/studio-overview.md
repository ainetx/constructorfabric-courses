...
| 2:25-3:05 | If the decision is still open, `cf-brainstorm` helps compare options and tradeoffs. Once the direction is clear, design workflows capture the design and Architecture Decision Record workflows record key architecture decisions. | Options board turns into DESIGN and Architecture Decision Record files. | Decide, then record |
